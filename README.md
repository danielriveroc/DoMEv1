...
This project contains the source code of the first version of the DoME algorithm for Symbolic Regression, written in Julia v1.4.2. The aim of this code is to be able to repeat the experiments described in the paper available at https://doi.org/10.1016/j.eswa.2022.116712. For this reason, this code will not be updated. Instead, a Julia package with the most recent code can be found at https://github.com/danielriveroc/SymDoME.jl, you can use this package to use this algorithm for Symbolic Regression and Classification.
However, the code in this project is fully functional. Feel free to use this source code to perform your experiments. If any publication is generated through this system, please add a citation to that paper. Also, if you need any more explanations on how to run DoME or how to repeat the experiments, or there is any issue with this repository, please let me know.
...
In any case, all the experiments described in the article are carried out in this way, with all values of the hyperparameters, performing a grid search. When experiments are finished, run the file "readResults.jl" to get Tables 5, 6, 7 and 8, and Figures 5, 6, 7 and 8. At the beginning of this file you can change a Boolean variable value to get the results of the Tables as latex strings or usual text.

However, the run of all of the experiments in the grid search will be computationally time-consuming. If you only want to check the values of Table 5, run the file called "exampleTable5.jl". In the first lines you will find all of the datasets with their hyperparameter configuration as shown in Table 5. Just uncomment the line corresponding to the one you want to repeat, and run this file. In this way, 10 experiments (10 folds) will be performed, and the median value of these 10 folds will be shown at the end. For each fold, an additional check is performed in this file, running the obtained expression with the dataset and ensuring that the MSE in test is the same as the one returned by the dome function. The content of this file is as follows:
